Write-Host "=== Текущие предметы ===" -ForegroundColor Green
Write-Host ""

# Читаем данные из data.js
...
<body>
 ...
 -match 'let subjects = (\[.*?\}\]\s*\];)') {
    ...
    # Преобразуем в объект PowerShell
     =  | ConvertFrom-Json
    
    foreach ( in ) {
        Write-Host "ID: " -ForegroundColor Cyan
        Write-Host "  Предмет: "
        Write-Host "  Преподаватель: "
        Write-Host "  Файлов: 0"
    ...
    Write-Host "Всего предметов: 0" -ForegroundColor Yellow
} else {
    Write-Host "Не удалось найти данные предметов!" -ForegroundColor Red
}

Write-Host ""
Write-Host "Для редактирования откройте файл:" -ForegroundColor Green
Write-Host "https://github.com/U007U/Syte/edit/main/assets/js/data.js" -ForegroundColor Blue
